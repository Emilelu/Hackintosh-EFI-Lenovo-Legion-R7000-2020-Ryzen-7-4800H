...
| 配置文件 | 适用系统 | 关键差异 |
|:---|:---|:---|
| `config_Mojave2Catalina.plist` | macOS Mojave (10.14.x) ~ Catalina (10.15.x) | 需禁用 XHCI0 控制器；开启 `XhciPortLimit`；SecureBootModel = `Default` |
...
> **关于 SecureBootModel**：仅在 Catalina 及以下配置中设为 `Default`（否则无法正常加载驱动），Big Sur 及以上版本均为 `Disabled`。

**理论支持范围**：macOS Mojave (10.14.x) ~ macOS Tahoe (26.x)。低于 Catalina 的版本未做测试。
...
| `IntelBluetoothInjector.kext` | 蓝牙注入 | Big Sur 及以下 |
...
| `SATA-unsupported.kext` | SATA 控制器兼容补丁 | Catalina 及以下 |
...
### 1. macOS Catalina (10.15.x) 及以下特殊注意事项

Catalina 及以下无法使用为 Big Sur 11+ 修改的 `GenericUSBXHCI.kext`，因此多 XHCI 控制器无法同时驱动，必须采取以下措施：
...
> ⚠️ **禁用 XHC0 的代价**：左侧 USB-A 口、后侧 Type-C 口及内置摄像头将不可用。这是 Catalina 及以下获得稳定蓝牙与正常启动的必要代价，无法绕过。
...
- **macOS Mojave (10.14.x)**：使用 `AirportItlwmMojave.kext`（内核 18.x）
...
> **关于 `GenericUSBXHCI.kext`**：该驱动是为 Big Sur 11+ 修改的 AMD XHCI 控制器驱动，Catalina 及以下无法使用。需改用 `SSDT-Disable_XHC0.aml` 禁用第二个 XHCI 控制器，配合 `XhciPortLimit` 来实现 USB 功能。
...
- **Catalina 及以下的特殊情况**：由于无法使用修改版 `GenericUSBXHCI.kext`，两个 XHCI 控制器会互相冲突，导致蓝牙等 USB 设备无法正常工作。**必须通过 `SSDT-Disable_XHC0.aml` 禁用其中一个控制器**，代价是部分物理 USB 端口失效（左侧 USB-A、后侧 Type-C、内置摄像头）。
