...
| `AirportItlwmAboveSequoia.kext` | 社区构建（itlwm 官方 release 无 Sequoia+ 版本），来源与内核绑定严格 |
...
- **已是最新**：VoodooI2C 2.9.1、YogaSMC 1.5.3、USBToolBox 1.2.0、HoRNDIS 9.2（官方早已停更）、RealtekRTL8111 3.0.x、AirportItlwm 系列（= itlwm v2.3.0 stable，最新 release）。
...
[PC711Probe](https://github.com/hrx114514x/PC711Probe)（v1.8.1）为 Lilu 插件，修复 SK hynix PC711 / BC711 / BC511 与三星 PM991 在 macOS 11–15 上的 NVMe Identify 超时 panic（macOS 26 已原生支持 PC711）。已在 AMD 平台 + macOS 15.6.1 完成全新安装与 96 GiB 读写压测，效果可信——**但仅限其验证过的硬盘型号**。

- 本机出厂盘为 **PC611，不在验证列表**，系统盘 SN570 亦无此问题，故默认 `Enabled=False`。
- 若你的出厂盘恰为 PC711 / BC711 / BC511 / PM991：在 config 的 Kernel → Add 中启用 `PC711Probe.kext`，并禁用 `SSDT-Disable_NVMe_GPP1.aml` 即可免屏蔽使用原硬盘。标准版与 Force 版不可同时启用。
